...
THRESHOLD_THUMB_CONDITION = 80  # Utilizad para determinar se dedo está 'esticado' ou 'dobrado'
...
def verify_hand_direction(hand):
    ...
    Para isso são verifica-se os valores das bases dos dedos e compara-se com o valor do pulso.
    """
...
def verify_hand_side_foward(label, hand, hand_direction):
    if label == "right":
        ...
        if hand_direction == "up":
            # Base dos dedos (valor da vertical, y)
            # pos_thumb = hand.landmark[1].x  # Polegar
...
def verify_hand_finger_condition(finger, points_y, hand_direction, hand_fingers_points):
    ...
    # TODO: [melhoria para vários casos]
    #  melhorar função deixar estado "esticado" mais bem explicado ? Ideia:
    ...
    # TODO: limitação -> não considera caso para mão de lado
...
def verify_adjacent_finger(states_, finger_name, adjacent_finger, hand_fingers_points):
    ...
    if finger_name in ['thumb']:  # Polegar
        # Distãncia entre ponta do polegar e base do dedo indicador
        # finger_distance = abs(hand_fingers_points[adjacent_finger]['x'][3] - hand_fingers_points[finger_name]['x'][3])
        thumb_distance = compute_distance_adjancent_finger_thumb(hand_fingers_points=hand_fingers_points)
        # Cálculo
        # distance = thumb_distance / reference_distance * 100
        ...
        if distance <= THRESHOLD_THUMB_DISTANCE:
            ...
            # Afastado e do dedo adjacente
            state_adjacent_finger = f"away from {adjacent_finger}"
        else:
            # Mais afastado do dedo adjacente
            state_adjacent_finger = f"far away from {adjacent_finger}"
    else:
        # Distãncia entre ponta dos dedos
        # finger_distance = abs(hand_fingers_points[adjacent_finger]['x'][3] - hand_fingers_points[finger_name]['x'][3])
...
def verify_hand_fingers_states(hand, img_height, hand_direction, hand_fingers_points):
    ...
    for i, finger in enumerate(HAND_FINGERS):
        ...
        state_list = verify_adjacent_finger(states_=finger_states_copy, finger_name=finger,
                                            adjacent_finger=adjacent_finger, hand_fingers_points=hand_fingers_points)  # TODO: acompanhar
